...
# Monitoraggio continuo del sistema live betting

Write-Host "`n📊 MONITORAGGIO LOG CONTINUO - LIVE BETTING`n" -ForegroundColor Cyan
    ...
    try {
        ...
        if ($content -and $content.Length -gt $lastPosition) {
            ...
            foreach ($line in $newLines) {
                ...
                # Opportunità filtrate per EV
                elseif ($line -match "valore atteso.*< soglia|EV.*<.*soglia|Saltata opportunità.*valore") {
                    $filteredCount++
                    ...
                    } elseif ($thresholdMatch -eq "10.0" -or $thresholdMatch -eq "10") {
                        Write-Host "⚠️  FILTRATO (soglia 10% - da correggere): " -ForegroundColor Red -NoNewline
                        Write-Host "EV $evMatch% minore di 10%" -ForegroundColor Red
                    } else {
